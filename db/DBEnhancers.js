...
export class Client_rep_DB_decorator {
  #dbRepo;
  #filter;
  #sort;

  constructor(dbRepo, filter = null, sort = null) {
    this.#dbRepo = dbRepo;
    this.#filter = filter;
    this.#sort = sort;
  }
  ...
  async get_k_n_short_list(k, n) {
    ...
    if (this.#sort?.field) {
      query += ` ORDER BY ${this.#sort.field} ${this.#sort.direction || "ASC"}`;
    } else {
    ...
    const res = await this.#dbRepo.query(query, params);
